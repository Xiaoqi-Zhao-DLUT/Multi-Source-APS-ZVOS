...
def main():
    ...
    train(net, model1, model2, optimizer)


def train(net, model1, model2, optimizer):
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                output1_videosal = model2(e1_rgb, e2_rgb, e3_rgb, e4_rgb, e5_rgb, output4_d, output3_d, output2_d, output1_d, output4_sal, output3_sal, output2_sal, output1_sal, flows) #hed
            score = net(inputs,flows,output1_sal,output1_videosal)
            score = score.reshape(batch_size,1)

            score_MAE_static_sal = torch.mean(torch.abs((output1_sal -  labels)),[2,3])
            score_MAE_video_sal = torch.mean(torch.abs((output1_videosal -  labels)),[2,3])


            labels_score = torch.ones(batch_size,1)
            labels_score[score_MAE_static_sal<score_MAE_video_sal] = 0
            labels_score = Variable(labels_score).cuda()
            optimizer.zero_grad()

            loss1 = criterion_BCE(score,labels_score)
            total_loss = loss1
            total_loss.backward()
            optimizer.step()
